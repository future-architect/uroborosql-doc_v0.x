...
また <Badge text="0.26.9+" vertical="middle"/>から、引数で指定された複数のSQLを単一の通信でまとめてデータベースに発行する `updateChained` APIが提供されています。これにより、ネットワークの往復によるオーバーヘッドを最小限に抑えることが可能です。

| 利用メソッド                                        | 説明                                                                                         |
| :-------------------------------------------------- | :------------------------------------------------------------------------------------------- |
| SqlAgent#update("[SQL名]")                          | [SQL名](./index.md#sql名)で説明した`SQL名`を指定                                             |
| SqlAgent#updateWith("[SQL文字列]")                  | `SQL文字列`を直接指定                                                                        |
| SqlAgent#updateChained("[SQL名1]", "[SQL名2]", ...) | 指定した複数のSQL名に対するSQLをまとめて発行する。 <Badge text="0.26.9+" vertical="middle"/> |
